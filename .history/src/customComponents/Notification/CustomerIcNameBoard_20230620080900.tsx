...
const CustomerIcNameBoard: React.FC<CustomerIcNameBoardProps> = ({
  ...
  const backgroundHue = adjustHue(0.1, idTypeBgColour); // Extract the hue from the background color
  const greyColour = "rgba(20,15,20, 0.2)"; // Replace with your desired grey color value
  const darkerColour = mix(0.55, backgroundHue, greyColour); // Adjust the darkness level (0.2) as per your preference
