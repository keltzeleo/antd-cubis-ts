...
const DragDropArea2: React.FC = () => {
  ...
  const handleError = (errorMsg: string) => {
    setErrorMessage((prevErrors) => [...prevErrors, errorMsg]);
  ...
  const handleChange = async (info: UploadChangeParam<UploadFile<any>>) => {
    ...
    if (isFileRedundant) {
      ...
      const errorMessage = `Redundant files won't be replaced. To reupload, please delete the existing ones:\n${redundantFileNames.join(
        "\n"
      )}`;
    ...
    if (duplicateFiles.length > 0) {
      ...
      handleError(
        `File(s) '${duplicatedFileNames.join(", ")}' ${
          duplicatedFileNames.length > 1 ? "are" : "is"
        } duplicated. Removed duplicated file(s). Please double-check.`
      );
      fileList = fileList.filter(
