  ...
  const handleChange = async (info: UploadChangeParam<UploadFile<any>>) => {
    ...
    if (redundantFiles.length > 0) {
      fileList = fileList.filter(
        (newFile) => newFile.uid !== redundantFiles[0].uid
      );
      message.error(
        `${redundantFiles.length} file(s) are redundant. Only one file allowed. Removed: ${redundantFiles[0].name}`
      );
    }
    ...
    if (duplicateFiles.length > 0) {
      ...
      message.error(
        `${duplicateFiles.length} file(s) are duplicates. Only one file allowed. Removed: ${duplicateFiles[0].file.name}`
      );
    }
