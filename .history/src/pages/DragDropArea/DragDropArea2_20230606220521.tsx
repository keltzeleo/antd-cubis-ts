  ...
  const props = {
    ...
    beforeUpload: (file: File) => {
      ...
      const updatedFileList = fileList.filter(
        (existingFile: any) => !existingFile.redundant // Remove redundant files from the fileList
      );
      ...
      if (file.type.includes("image")) {
        ...
        reader.onloadend = () => {
          ...
          setFileList([updatedFile, ...updatedFileList]); // Add the updated file to the fileList
        };
        return false; // Skip the file from being added to the fileList immediately
      }

      // Add redundant property to the file object if it is redundant
      if (isFileRedundant) {
        setFileList([...updatedFileList, { ...file, redundant: true }]);
