...
const beforeUpload = async (file: RcFile, fileList: UploadFile<any>[]) => {
  const isFileRedundant = fileList.some(async (existingFile) => {
    // Compare the checksum of the existing file and the file being uploaded
    const existingChecksum = await getChecksum(existingFile.originFileObj as RcFile);
    const newChecksum = await getChecksum(file);
...
const DragDropArea2: React.FC = () => {
  ...
  onst handleChange: UploadProps<UploadFile<any>>["onChange"] = ({
