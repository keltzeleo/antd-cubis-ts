...
const DragDropArea2: React.FC = () => {
  ...
  const handleChange = async (info: UploadChangeParam<UploadFile<any>>) => {
    ...
    // Update the handleError function calls
      ...
      >
        <Upload.Dragger
          ...
          onDrop={(e) => {
            ...
            // Update the type of unsupportedFiles as RcFile[]
            const unsupportedFiles: RcFile[] = Array.from(
              e.dataTransfer.files
            ).filter((file) => !acceptedFileTypes.includes(file.type));

            // Update the handleError function call for unsupportedFiles
            if (unsupportedFiles.length > 0) {
              const unsupportedUploadFile: UploadFile<any> = {
                uid: "unsupportedFile-" + Date.now().toString(),
                name: unsupportedFiles[0].name,
                size: unsupportedFiles[0].size,
                type: unsupportedFiles[0].type,
                lastModified: unsupportedFiles[0].lastModified,
                lastModifiedDate: unsupportedFiles[0].lastModifiedDate,
                originFileObj: unsupportedFiles[0],
              };

              handleError(
                "Unsupported file type. Please upload a valid file.",
                unsupportedUploadFile
              );
            }
          }}
