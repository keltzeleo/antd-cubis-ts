...
const MainTable = () => {
  ...
  const filterData = (rowData: RowData[], filterStatus: string[]) => {
    return rowData.filter((row) => row.status.some((status) => filterStatus.includes(status)));
  };
  ...
  return (
    <>
      ...
      {filteredDataNested.length > 0 && (
        <Table columns={columns} dataSource={filteredDataNested} pagination={false} />
      )}
      <div>
        {data.map((row, index) => (
          <div key={index}>
      ...
      {filteredDataMaster.length > 0 && (
        <Table columns={columns} dataSource={filteredDataMaster} pagination={false} />
      )}
      <div>
        {data.map((row, index) => (
          <div key={index}>
