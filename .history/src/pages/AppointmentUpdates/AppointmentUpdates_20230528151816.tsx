...
interface FilterGroupProps {
  ...
  onChange: (values: StatusLabels[]) => void;
}

const FilterGroup: React.FC<FilterGroupProps> = ({ options, onChange }) => {
  const handleCheckboxChange = (checkedValues: CheckboxValueType[]) => {
    onChange(checkedValues as StatusLabels[]);
  };
...
const [selectedStatus, setSelectedStatus] = useState<StatusLabels[]>([]);

const handleStatusFilter = (values: StatusLabels[]) => {
  setSelectedStatus(values);
};

const availableStatusOptions: StatusLabels[] = [
...
<FilterGroup options={availableStatusOptions} onChange={handleStatusFilter} />;
