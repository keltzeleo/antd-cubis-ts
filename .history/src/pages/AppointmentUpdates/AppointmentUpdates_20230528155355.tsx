...
import { Avatar, Button, Checkbox, ConfigProvider, Drawer, Table, Tag } from "antd";
...
const AppointmentUpdates: React.FC = () => {
  ...
  const [addAppointmentDrawerVisible, setAddAppointmentDrawerVisible] = useState(false);
  const [statusFilters, setStatusFilters] = useState<{ [key: string]: StatusLabels[] }>({
    "1": [StatusLabels.ASSIGNED, StatusLabels.CANCELLED, StatusLabels.FAILED_TO_VISIT, StatusLabels.REASSIGNING, StatusLabels.RESCHEDULED],
    "2": [StatusLabels.ASSIGNED, StatusLabels.CANCELLED, StatusLabels.FAILED_TO_VISIT, StatusLabels.REASSIGNING, StatusLabels.RESCHEDULED],
    "3": [StatusLabels.ASSIGNED, StatusLabels.CANCELLED, StatusLabels.FAILED_TO_VISIT, StatusLabels.REASSIGNING, StatusLabels.RESCHEDULED],
  });
  ...
  const tags = [
    ...
    { value: "failed to visit", label: "Failed to Visit", color: light["orange"] },
  ...
  const expandedRowRender = (record: Plumber) => {
    ...
    const nestedColumns = [
...

      {
        ...
        <select
        value={statusFilters}
        onChange={(e) => setStatusFilters(Array.from(e.target.selectedOptions, (option) => option.value))}
        multiple
      >
        filterDropdown: () => (
          <div>
            <Checkbox.Group
              ...
              <Table
              dataSource={filteredAppointmentsByStatus}
    ...
    plumberStatusFilters.includes(appointment.status)
  );
      ...
      render: (name: string, record: Plumber) => {
        ...
        return (
          <>
            ...
            <Table
        dataSource={filteredAppointmentsByTags}
        columns={nestedColumns}
        pagination={false}
      />
          </>
          ...
          title={
            <div>
              <Avatar
                ...
                style={{ backgroundColor: getRandomColor(), marginLeft: -2, marginRight: 16 }}
              />
              {drawerData?.name}
            </div>
          }
