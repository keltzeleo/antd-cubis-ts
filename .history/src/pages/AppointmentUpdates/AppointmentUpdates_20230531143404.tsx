...
const AppointmentUpdates: React.FC = () => {
  ...
  const SearchComponent: React.FC = () => {
    ...
    const handleSearch = (value: string) => {
      ...
      const results = ['Plumber A', 'Plumber B', 'Plumber C'].filter(plumber =>
        plumber.toLowerCase().includes(value.toLowerCase())
      );
  ...
  return (
    <ConfigProvider theme={{ token: light }}>
      <div style={{ display: "flex", alignItems: "center" }}>
        <div style={{ marginRight: "16px" }}>Plumber Search:</div>
        <Form.Item style={{ marginBottom: "0" }}>
          <Input.Search
            style={{ marginTop: "32px", width: "20vh" }}
            allowClear
            enterButton="Search"
            size="large"
            onSearch={onSearch}
            placeholder="Enter Plumber's Name or ID"
          />
        </Form.Item>
      </div>
      <p></p>
