...
const AppointmentUpdates: React.FC = () => {
  ...
  data.forEach((plumber) => {
    ...
    plumber.unassignedAppointments = plumber.appointments.filter(appointment => appointment.status === "pending").length;
  });
...
{record.appointments.filter(appointment => appointment.status === "cancelled").length} pending
