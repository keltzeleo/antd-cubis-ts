...
const NapsWizard2: React.FC<{ theme: Theme }> = ({ theme }) => {
  ...
  const renderCircularProgress = () => {
    ...
    const stepsPopoverContent = (
      <div>
        <Typography.Text strong>Completed Steps:</Typography.Text>
        <div>
          {completedStepsList.map((step) => (
            <Button
              key={step}
              onClick={() => handleStepClick(step - 1)} // Link to the relevant step
              style={{ marginRight: 8 }}
              type="link"
            >
        ...
        <div>
          {pendingStepsList.map((step) => (
            <Button
              ...
              type="link"
            >
              <span style={{ color: theme["grey.6"] }}>Step {step}</span>
            </Button>
          ))}
        </div>
      </div>
