...
const NapsWizard2: React.FC<{ theme: Theme }> = ({ theme }) => {
  const napsWizardRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    if (napsWizardRef.current) {
      observer.observe(napsWizardRef.current);
    }

    return () => {
      if (napsWizardRef.current) {
        observer.unobserve(napsWizardRef.current);
      }
    };
  }, []);
  ...
  const calculateInitialStickyPosition = () => {
    // Use "calc" CSS function to dynamically calculate the position for the sticky footer
    return `calc(100vh - ${
      napsWizardRef.current?.getBoundingClientRect().top
    }px)`;
  };

  return (
    <div>
      <PageContainer footer={undefined}>
        {" "}
        {/* Disable the default PageContainer footer */}
        <div ref={napsWizardRef} className="steps-container">
          {/* ... rest of the code ... */}
        </div>
        <div
          style={{
            position: stickyVisible ? "fixed" : "relative",
            top: calculateInitialStickyPosition(),
            bottom: 0,
            left: 0,
            right: 0,
            marginLeft: -16,
