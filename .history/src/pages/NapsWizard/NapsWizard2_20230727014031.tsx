...
const NapsWizard2: React.FC<NapsWizard2Props> = ({ theme }) => {
  ...
  // 函數用來動態顯示步驟的圖示和數字，這裡會加入兩個新的圖案變化，分別是⓶和⓷
  const getStepDisplay = (stepIndex: number) => {
    if (stepIndex < completedSteps) {
      ...
      return <span className="step-number">{completedSteps + 3 - stepIndex}</span>;
    }
  };
      ...
      <div className="steps-container">
        ...
        <div className="current-step">
          ...
          <div className="step-title">
            {currentStep < totalSteps ? Step[currentStep].props.title : ""}
          </div>
        </div>
        <div className="pending-steps">
          {Array.from({ length: Math.min(pendingSteps, 2) }, (_, index) => completedSteps + index + 1).map(stepIndex => (
            <div key={stepIndex} className="step">
              {getStepDisplay(stepIndex)}
            </div>
          ))}
          {pendingSteps > 2 && (
