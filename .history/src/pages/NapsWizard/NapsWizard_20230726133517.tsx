...
const generateCompletedStepsDisplay = (completed: number, total: number) => {
  ...
  for (let i = 1; i <= completed; i++) {
    display.push(i === completed ? `⓵` : `(${i}`);
  }
  return display.join(" ");
};

// 获取已完成步骤的显示
const completedStepsDisplay = generateCompletedStepsDisplay(completedSteps, totalSteps);
...
const generateRemainingStepsDisplay = (current: number, total: number) => {
  ...
  for (let i = current + 1; i <= total; i++) {
    display.push(i === current + 1 ? `⓸` : `(${i}`);
  }
  return display.join(" ");
};

// 获取尚未完成步骤的显示
const remainingStepsDisplay = generateRemainingStepsDisplay(currentStep, totalSteps);
  ...
  <>
    ...
    >
      <div className="naps-wizard-container">
        {/* 左侧已完成步骤显示 */}
        <div className="naps-wizard-completed-steps">{completedStepsDisplay}</div>

        {/* 中间当前步骤显示 */}
        <div className="naps-wizard-current-step">
          <div className="naps-wizard-step-title">({currentStep} 步骤标题)</div>
          {currentStepFormContent}
        </div>

        {/* 右侧尚未完成步骤显示 */}
        <div className="naps-wizard-remaining-steps">{remainingStepsDisplay}</div>
