...
const NapsWizard: React.FC<NapsWizardProps> = ({ theme }) => {
  ...
  // 生成尚未完成步骤的显示
  const generateRemainingStepsDisplay = (current: number) => {
    const display = [];
    for (let i = current + 1; i <= totalSteps; i++) {
      display.push(`❹❺⓸`.charAt(i - current - 1));
    }
    return display.join(" ");
  };

  // 获取尚未完成步骤的显示
  const remainingStepsDisplay = generateRemainingStepsDisplay(currentStep);
