...
const NapsWizard: React.FC<NapsWizardProps> = ({ theme }) => {
  ...
  const activeStepArray = [activeStep];
  ...
  const transitions = useTransition(activeStepArray, {
    from: { opacity: 0, transform: "rotateX(90deg)" },
    enter: { opacity: 1, transform: "rotateX(0deg)" },
    leave: { opacity: 0, transform: "rotateX(-90deg)" },
    config: config.default, // Ensure config is imported correctly
  }) as any; // Use 'as any' as a temporary fix
  ...
  return (
    <div className="wizard-container">
      ...
      <div className="form-container wizard-form-container">
        ...
        {transitions.map(({ item, key, props }: Transition) => (
