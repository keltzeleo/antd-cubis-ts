...
import { Button, Steps, Tooltip, message } from "antd";
...
const NapsWizard2: React.FC<{ theme: Theme }> = ({ theme }) => {
  ...
  const [showStepList, setShowStepList] = useState(false); // Added state for showing/hiding the step list
  ...
  const renderCircularProgress = () => {
    const diameter = 60; // Adjust the diameter as needed
    ...
    return (
      <div className="circular-progress-container">
        <svg className="circular-progress" height={diameter} width={diameter}>
          <circle
  ...
  const renderStepList = () => {
    const stepItems = forms.map((FormComponent, index) => (
  ...
  return (
    <div>
      <PageContainer>
        ...
        {/* Add the Tooltip to show the step list on hover */}
        <Tooltip
          placement="right"
          title={renderStepList()}
          visible={showStepList}
          onVisibleChange={setShowStepList}
        >
          <div
            className="circular-progress-container"
            onMouseEnter={() => setShowStepList(true)}
            onMouseLeave={() => setShowStepList(false)}
          >
            <svg
