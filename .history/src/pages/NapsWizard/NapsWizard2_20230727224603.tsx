    ...
    return (
      <div className="circular-progress-container">
        ...
        >
          ...
          />
          {/* Add lines connecting each step */}
          {Array.from({ length: totalSteps }).map((_, index) => {
            const angle = index * stepAngle;
            const x1 =
              radius + (radius - 3) * Math.cos((angle * Math.PI) / 180);
            const y1 =
              radius + (radius - 3) * Math.sin((angle * Math.PI) / 180);
            const x2 =
              radius + (radius - 10) * Math.cos((angle * Math.PI) / 180);
            const y2 =
              radius + (radius - 10) * Math.sin((angle * Math.PI) / 180);
            return (
              <line
                key={index}
                x1={x1}
                ...
                y2={y2}
                strokeWidth="1"
                stroke={theme["cyan.3"]} // Adjust the color as needed
              />
            );
          })}
        </svg>
