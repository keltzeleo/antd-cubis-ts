...
const mockData: DataSourceType[] = [
  {
    ...
    itemQuantity: 5,
    itemChargeRate: 20.0,
    itemAmount: 5 /* itemQuantity */ * 20.0 /* itemChargeRate */,
  ...
  },
  {
    ...
    itemQuantity: 3,
    itemChargeRate: 15.0,
    itemAmount: 3 /* itemQuantity */ * 15.0 /* itemChargeRate */,
...
mockData.forEach((item) => {
  const quantity = parseFloat(item.itemQuantity as string);
  const chargeRate = parseFloat(item.itemChargeRate as string);

  if (!isNaN(quantity) && !isNaN(chargeRate)) {
    item.itemAmount = (quantity * chargeRate).toFixed(2);
  } else {
    ...
    item.itemAmount = "N/A"; // Or any other suitable default value
  }
});
...
const StationBill: React.FC<StationBillProps> = ({ theme }) => {
  ...
  const columns: ProColumns<DataSourceType>[] = [
...

    {
      ...
      render: (text, record) => (
        <span style={{ color: theme.colorText }}>
          RM {(record.itemAmount || 0).toFixed(2)}
        </span>
      ),
    },
    {
