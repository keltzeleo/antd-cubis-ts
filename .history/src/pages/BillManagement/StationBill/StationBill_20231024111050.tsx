...
const StationBill: React.FC<StationBillProps> = ({ theme }) => {
  ...
  const columns: ProColumns<DataSourceType>[] = [
    ...
    },
    {
      ...
      render: (text, record) => {
        // Calculate itemAmount based on the values of itemQuantity and itemChargeRate
        const calculateItemAmount = (
          ...
          const parsedChargeRate =
            typeof chargeRate === "number"
              ? chargeRate
              : typeof chargeRate === "string"
              ? parseFloat(chargeRate)
              : NaN;

          if (!isNaN(parsedQuantity) && !isNaN(parsedChargeRate)) {
            return (parsedQuantity * parsedChargeRate).toFixed(2);
          } else {
            return "NaN";
          }
        };

        // Check if the current record is in edit mode
        const isEditing = record.id === editingKey;
        
        if (isEditing) {
          const itemQuantity = form.getFieldValue("itemQuantity"); // Remove the array brackets
          const itemChargeRate = form.getFieldValue("itemChargeRate"); // Remove the array brackets
          const itemAmount = calculateItemAmount(itemQuantity, itemChargeRate);
          
          return (
            <span style={{ color: theme.colorText }}>{`RM ${itemAmount}`}</span>
          );
        }
