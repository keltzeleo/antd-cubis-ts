...
const TableTransfer = ({
  leftColumns,
  rightColumns,
  selectedKeys, // Add selectedKeys as a prop

  ...restProps
}: TableTransferProps & { selectedKeys: string[] }) => (
  <Transfer<DataType>
    {...restProps}
  ...
  const leftTableColumns: ColumnsType<DataType> = [
    ...
    },
    {
      title: (
        <Checkbox
          checked={
            selectedKeys.length > 0 && selectedKeys.length === mockData.length
          }
          onChange={(e) => {
      ...
      render: (text, record) => (
        <Checkbox
          defaultChecked={selectedKeys.includes(record.key)} // Add the defaultChecked prop
  ...
  return (
    <>
      ...
      <TableTransfer
        ...
        selectedKeys={selectedKeys} // Pass the selectedKeys here
      />
    </>
  );
};
