...
const WaterBooksScheduler: React.FC<WaterBooksSchedulerProps> = ({ theme }) => {
  ...
  // Create a new instance of DateHolidays and load Malaysia's holidays
  const malaysiaHolidays = new DateHolidays('MY');
  malaysiaHolidays.init();
 ...
 const isMalaysiaHoliday = malaysiaHolidays.isHoliday(date.toDate());
 ...
 if (isMalaysiaHoliday) {
   cellStyle.backgroundColor = theme.holidayBackgroundColor;
 } else if (isWeekend) {
   cellStyle.backgroundColor = theme.weekendBackgroundColor;
 }

 return (
   <ul className="events">
     {isWeekend && (
       <div style={{ color: theme.colorTextSecondary, marginBottom: 5 }}>
         Rest Day
       </div>
     )}
     <Droppable droppableId={date.format('DD-MM-YYYY')}>
       {(provided) => (
         <div ref={provided.innerRef} {...provided.droppableProps} style={cellStyle}>
