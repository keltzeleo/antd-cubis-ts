...
const WaterBooksScheduler: React.FC<WaterBooksSchedulerProps> = ({ theme }) => {
  ...
  // Check if it's Sunday (0) or Saturday (6) and apply custom class accordingly
  const customClass = current.day() === 0 || current.day() === 6 ? 'backgroudColor:theme["red.2"]' : '';

  return (
    <div className={`ant-picker-cell ${customClass}`}>
      {info.originNode}
    </div>
  );
};
