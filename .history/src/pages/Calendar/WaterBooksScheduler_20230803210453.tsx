            ...
            return (
              <React.Fragment key={`month_${monthIndex}`}>
                ...
                {Array.from({ length: monthDaysInMonth }, (_, dayIndex) => {
                  ...
                  // Find the corresponding legend item based on the date category
                  const legendItem = legendData.find(
                    (item) => item.category === getDayCategory(date)
                  );

                  // Set the background color based on the legend category
                  const backgroundColor = legendItem
                    ? legendItem.color
                    : "transparent";
