...
const WaterBooksScheduler: React.FC<WaterBooksSchedulerProps> = ({ theme }) => {
  ...
  const renderSingleRowCalendar = (
    ...
    handleRightTableColumnDateChange: Dayjs | null // Pass selectedRightTableColumnDate as a parameter
  ) => {
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <div>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          {showSingleRow ? (
            renderSingleRowCalendar(selectedDate, handleSingleRowDateSelect)
          ) : (
            <Calendar
              value={value}
              onSelect={(date) => {
