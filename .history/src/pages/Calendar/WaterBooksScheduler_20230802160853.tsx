...
const WaterBooksScheduler: React.FC<WaterBooksSchedulerProps> = ({ theme }) => {
  ...
  const [malaysiaHolidays, setMalaysiaHolidays] = useState<DateHolidays | null>(null);
  ...
  useEffect(() => {
    const fetchMalaysiaHolidays = async () => {
      try {
        ...
        setMalaysiaHolidays(malaysiaHolidays);
      } catch (error) {
  ...
  const dateCellRender = (date: Dayjs) => {
    ...
    const isMalaysiaHoliday = malaysiaHolidays?.isHoliday(date.toDate()) ?? false;

    return (
      <ul className="events">
        {isWeekend && (
          <div style={{ color: theme.colorTextSecondary, marginBottom: 5, backgroundColor: theme["red.4"], borderRadius: 8, paddingLeft: 8, }}>
            Rest Day
          </div>
        )}
        {isMalaysiaHoliday && (
          <div style={{color: theme.colorTextSecondary, backgroundColor: theme["geekblue.3"], marginBottom: 5 }}>
            Holiday
          </div>
        )}
