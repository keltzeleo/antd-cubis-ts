...
const WaterBooksScheduler: React.FC<WaterBooksSchedulerProps> = ({ theme }) => {
  const [scheduledBooks, setScheduledBooks] = useState<Record<string, { type: string; content: string }[]>>(mockScheduledBooks); // Set the initial state with the mock data
  const [value, setValue] = useState<Dayjs>(dayjs()); // Set the initial value to today's date
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    // Add this check to ensure scheduledBooks is not undefined
    if (!scheduledBooks) {
      return;
    }
