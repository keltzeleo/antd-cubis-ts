...
const renderSingleRowCalendar = () => {
  ...
  const startMonth = value.clone().subtract(numMonthsToShow - 1, "month");
  const endMonth = value.clone().add(numMonthsToShow - 1, "month");
  ...
  return (
    ...
    style={{
      ...
      scrollbarColor: "darkgray lightgray", // For browsers that support scrollbar color
    }}
  >
      <div style={{ display: "flex", flexDirection: "row" }}>
        {/* Render only the months within the specified range */}
        {Array.from({ length: numMonthsToShow * daysInMonth }, (_, index) => {
          const date = startDate.clone().add(index, "days");
          const isToday = date.isSame(dayjs(), "day");

          if (index % daysInMonth === 0) {
            // Check if the current date is within the specified range
            if (date.isBefore(startMonth) || date.isAfter(endMonth)) {
              return null; // Skip rendering if the date is outside the range
            }
            // Add the month name to the first column
            return (
              <div
          ...
          return (
            <div
              ...
              style={{
                ...
                padding: 8,
                textAlign: "center",
                backgroundColor: isToday ? theme["cyan.4"] : "transparent",
                color: theme["colorText"],
                fontFamily: "play",
              }}
            >
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <div>
        ...
        >
          ...
          {showSingleRow ? "Switch to Month View" : "Switch to Single-Row View"}
        </Button>
      </div>
