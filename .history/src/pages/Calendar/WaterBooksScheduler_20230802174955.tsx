...
const WaterBooksScheduler: React.FC<WaterBooksSchedulerProps> = ({ theme }) => {
  ...
  const draggableMockScheduledBooks = scheduledBooks.map((event, index) => (
    <Draggable
      key={event.content}
      draggableId={event.content}
      index={index}
    />
  ));
  ...
  const dateCellRender = (date: Dayjs) => {
    ...
    return (
      <ul className="events">
        ...
        <Droppable droppableId="scheduledBooks">
          {(provided) => (
