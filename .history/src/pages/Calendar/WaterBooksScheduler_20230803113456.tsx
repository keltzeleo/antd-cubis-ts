...
const WaterBooksScheduler: React.FC<WaterBooksSchedulerProps> = ({ theme }) => {
  ...
  const fetchMalaysiaHolidays = async () => {
    try {
      ...
      // Instead of replacing the scheduledBooks state, merge the holiday data with the existing events
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    if (listData.length === 0) {
      ...
      if (sourceDateKey === destinationDateKey) {
        ...
        // Instead of passing a string to createEmptyEventArray, pass the destinationDate object
