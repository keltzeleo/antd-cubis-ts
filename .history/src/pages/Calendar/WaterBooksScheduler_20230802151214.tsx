...
// Define the legend colors and types
const legendColors: { [key: string]: string } = {
  scheduled: 'theme["yellow.2"]',
  unscheduled: 'theme["green.2"]',
  restDay: 'theme["red.2"]',
  holiday: 'theme["blue.2"]',
};
...
const WaterBooksScheduler: React.FC<WaterBooksSchedulerProps> = ({ theme }) => {
  ...
  const [value, setValue] = useState<Dayjs>(dayjs);
    ...
    // Fetch Malaysian holidays from the API
    const fetchMalaysianHolidays = async () => {
      try {
        const response = await axios.get(
          'https://raw.githubusercontent.com/AdlanArif/malaysia-public-holidays/main/2023/2023-my.json'
        );
        const holidays = response.data;
        // Now you have the holidays data in the "holidays" variable.
        // You can use this data to determine if a date is a holiday or not.
        console.log(holidays);
      } catch (error) {
        console.error('Failed to fetch Malaysian holidays:', error);
      }
    };
  
    // Fetch the Malaysian holidays when the component mounts
    useEffect(() => {
      fetchMalaysianHolidays();
    }, []);
  
    // Function to check if a date is a Saturday or Sunday (rest day)
    const isRestDay = (date: Dayjs): boolean => {
  ...
  const cellRender = (current: Dayjs, info: CellRenderInfo<Dayjs>) => {
    ...
    const isScheduled = getListData(current).length > 0;
    const isRest = isRestDay(current);

    const color = isScheduled
      ? legendColors.scheduled
      : isRest
      ? legendColors.restDay
      : legendColors.unscheduled;
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    // Remove the dragged item from the source date list
    const newStartList = [...start];
    newStartList.splice(source.index, 1);
  
    // Insert the dragged item into the destination date list
    const newFinishList = [...finish];
    newFinishList.splice(destination.index, 0, draggedItem);
  
    // Update the scheduledBooks state with the new lists
