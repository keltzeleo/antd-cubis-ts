...
const WaterBooksScheduler: React.FC<WaterBooksSchedulerProps> = ({ theme }) => {
  ...
  // Function to check if a date has events or is empty
  const isDateEmpty = (date: Dayjs): boolean => {
    const dateKey = date.format("DD-MM-YYYY");
    return !(dateKey in scheduledBooks) || scheduledBooks[dateKey].length === 0;
  };

  // Function to generate mock data for a date
  const generateMockDataForDate = (date: Dayjs): EventData[] => {
    const dateKey = date.format("DD-MM-YYYY");
    const mockData: EventData[] = [
      {
        ...
        bookNo: "B007",
        totalBooks: "3",
        bookDescription: "Mock Event round",
      },
      // You can add more mock events here if needed
    ];
    return mockData;
  };

  // Check if the date 05/08/2023 is empty and generate mock data if needed
  const dateToCheck = dayjs("09-08-2023", "DD-MM-YYYY");
  if (isDateEmpty(dateToCheck)) {
    const mockDataForDate = generateMockDataForDate(dateToCheck);
    setScheduledBooks({
      ...scheduledBooks,
      [dateToCheck.format("DD-MM-YYYY")]: mockDataForDate,
    });
  }
