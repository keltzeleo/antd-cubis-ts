...
const WaterBooksScheduler: React.FC<WaterBooksSchedulerProps> = ({ theme }) => {
  ...
  const eventRender = (date: moment.Moment) => {
    ...
    const dateEvents = events.filter((event) => event.date === momentDate.format('YYYY-MM-DD'));
        ...
        <Droppable droppableId="events">
          {(provided) => (
            <div ref={provided.innerRef} {...provided.droppableProps}>
              {events.map((event, index) => eventRender(dayjs(event.date)))}
              {provided.placeholder}
            </div>
          )}
        </Droppable>
      </DragDropContext>
      <Calendar dateCellRender={(date) => <div>{eventRender(date)}</div>} />
    </div>
  );
};
