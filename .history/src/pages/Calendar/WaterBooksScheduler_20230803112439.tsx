...
const WaterBooksScheduler: React.FC<WaterBooksSchedulerProps> = ({ theme }) => {
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    if (
      ...
      // If the destination date cell has an empty scheduledBooks array, create a new event for that date
      const newEvent = {
        id: generateUniqueID(),
      ...
      // Create an empty array for the destination date
      createEmptyEventArray(destinationDate);
      // Add the new event to the destination date
  ...
  const dateCellRender = (date: Dayjs) => {
    ...
    if (dateKey === "09-08-2023" && listData.length === 0) {
      return (
        <div
          style={{
