  ...
  const dateCellRender = (date: Dayjs) => {
    ...
    return (
      <Droppable droppableId={dateKey} key={dateKey}>
        {(provided) => (
          <ul className="events" ref={provided.innerRef} {...provided.droppableProps}>            {isToday && (
    ...
    {listData.map((item, index) => (
            <Droppable droppableId={`${dateKey}-${index}`} key={`${dateKey}-${index}`}>
              {(provided) => (
                <li
...
const onDragEnd =  (result: DropResult) => {
  ...
  if (destination.droppableId !== source.droppableId || destination.index !== source.index) {
    ...
    // Handle the case where an event is dropped onto another event within the same date
    const dateKey = source.droppableId;
    const bookList = Array.from(scheduledBooks[dateKey]);

    // Get the book that is being dragged
    const [draggedBook] = bookList.splice(source.index, 1);

    // Insert the dragged book at the new position in the list
    bookList.splice(destination.index, 0, draggedBook);

    // Create a new state object with the updated book list for the date
    const newScheduledBooks = {
      ...scheduledBooks,
      [dateKey]: bookList,
    };
