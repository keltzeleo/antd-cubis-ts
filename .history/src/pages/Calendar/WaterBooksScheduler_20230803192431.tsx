...
const WaterBooksScheduler: React.FC<WaterBooksSchedulerProps> = ({ theme }) => {
  ...
  const dateCellRender = (date: Dayjs) => {
    ...
    if (showSingleRow) {
      ...
      const singleRowDates = Array.from({ length: daysInMonth }, (_, index) =>
      startDate.add(index, "day")
    );

    return (
      <ul className="events" style={{ display: "flex" }}>
        {singleRowDates.map((singleDate) => (
          <li key={singleDate.format("YYYY-MM-DD")} className="date-cell">
            {singleDate.format("D")}
          </li>
        ))}
      </ul>
      ...
      <div>
        ...
        >
          ...
          <Button onClick={handleToggleSingleRow}>
            {showSingleRow ? "Show All Dates" : "Show Single Row"}
          </Button>
          <Alert
