...
const WaterBooksScheduler: React.FC<WaterBooksSchedulerProps> = ({ theme }) => {
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    if (source.droppableId === destination.droppableId) {
      const reorderedEvents = Array.from(startEvents);
      const [removedEvent] = reorderedEvents.splice(source.index, 1);
      reorderedEvents.splice(destination.index, 0, removedEvent);

      setScheduledBooks({
        ...scheduledBooks,
        [source.droppableId]: reorderedEvents,
      });
    } else {
      ...
      const [removedEvent] = startEventsCopy.splice(source.index, 1);
      finishEventsCopy.splice(destination.index, 0, removedEvent);

      setScheduledBooks({
        ...scheduledBooks,
        [source.droppableId]: startEventsCopy,
        [destination.droppableId]: finishEventsCopy,
      });
    }
  };
