...
const WaterBooksScheduler: React.FC<WaterBooksSchedulerProps> = ({ theme }) => {
  ...
  const dateCellRender = (date: Dayjs) => {
    ...
    return (
      <ul className="events">
        {isToday && (
          <div style={{ color: theme.colorTextSecondary, backgroundColor: theme.colorPrimary, marginBottom: 5, borderRadius: 8, paddingLeft: 8, fontSize:12, fontWeight:600 }}>
            Today
          </div>
        )}
        {isWeekend && (
  ...
  {(provided) => (
    <div ref={provided.innerRef} {...provided.droppableProps}>
      {listData.map((item, index) => (
        // Make sure `draggableId` is unique for each item
        <Draggable key={item.content} draggableId={item.content + index} index={index}>
          {(provided) => (
            <li
...
// The `source.droppableId` and `destination.droppableId` will be date strings
