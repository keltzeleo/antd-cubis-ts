...
const WaterBooksScheduler: React.FC<WaterBooksSchedulerProps> = ({ theme }) => {
  ...
  // Function to create empty mock data for event item
  const createEmptyEventItem = (date: string) => {
    return {
      id: generateUniqueID(),
      content: "", // Add the 'content' property and set it to an empty string
      date: date,
      reader: "",
      bookNo: "",
      totalBooks: "",
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    const isEmptyDropTarget =
      ...
      (!newScheduledBooks[destinationDateKey] ||
        newScheduledBooks[destinationDateKey].length === 0);
