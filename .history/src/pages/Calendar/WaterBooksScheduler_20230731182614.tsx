    ...
    return (
      <ul className="events">
        {listData.map((item, index) => (
          <li key={index} onClick={() => showDrawer(item.content)}> {/* Pass the item's title */}
            {item.content}
          </li>
        ))}
      </ul>
  ...
  return (
    <>
      ...
      <div>
        ...
        {/*  Drawer component */}
        <Drawer
        title={`Water Books Rescheduling - ${clickedItemTitle}`} {/* Display the title in the Drawer's title */}
        placement="right"
        closable={true}
