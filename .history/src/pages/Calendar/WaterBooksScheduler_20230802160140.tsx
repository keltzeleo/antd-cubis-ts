...
const WaterBooksScheduler: React.FC<WaterBooksSchedulerProps> = ({ theme }) => {
  ...
  const dateCellRender = (date: Dayjs) => {
    ...
    const cellClasses = classNames({
      'date-cell': true,
      'weekend': isWeekend,
      'holiday': isMalaysiaHoliday,
    });
  
    return (
      <ul className="events">
        <div className={cellClasses} onClick={() => onSelect(date)}>
          <div
            style={{
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'space-between',
              backgroundColor: isWeekend ? theme["red.4"] : 'transparent',
              borderRadius: 8,
              paddingLeft: 8,
            }}
          >
            ...
            {isWeekend && (
              <div style={{ color: theme.colorTextSecondary, marginLeft: 8 }}>
                Rest Day
              </div>
            )}
          </div>
