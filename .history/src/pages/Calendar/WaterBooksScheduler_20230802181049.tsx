...
const WaterBooksScheduler: React.FC<WaterBooksSchedulerProps> = ({ theme }) => {
  ...
  const dateCellRender = (date: Dayjs) => {
    ...
    return (
      <ul className="events">
        {/* your other div elements */}
        <Droppable droppableId={date.format('DD-MM-YYYY')}>
          {(provided) => (
            <div ref={provided.innerRef} {...provided.droppableProps}>
              {listData.map((item, index) => (
                <Draggable key={item.content} draggableId={`${item.content}-${date.format('DD-MM-YYYY')}`} index={index}>
                  {(provided) => (
                    <li
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    const finishDay = destination.droppableId;
    const eventId = draggableId.split('-')[0];

    const startEvents = scheduledBooks[startDay] || [];
    const finishEvents = scheduledBooks[finishDay] || [];

    const startEventsCopy = Array.from(startEvents);
    const finishEventsCopy = Array.from(finishEvents);

    const [removedEvent] = startEventsCopy.filter((event) => event.content === eventId);
    startEventsCopy.splice(startEventsCopy.indexOf(removedEvent), 1);
    finishEventsCopy.push(removedEvent);

    setScheduledBooks({
      ...scheduledBooks,
      [startDay]: startEventsCopy,
      [finishDay]: finishEventsCopy,
    });
  };
