...
const isMalaysiaHoliday = (date: Dayjs) => {
 ...
 const isToday = date.isSame(dayjs(), 'day'); // Check if the date is today
      ...
      {isToday && (
        <div style={{ color: 'white', backgroundColor: theme.colorPrimary, marginBottom: 5, borderRadius: 8, paddingLeft: 8, fontSize:12, fontWeight:600 }}>
          Today
              ...
              {isWeekend && (
          <div style={{ color: theme.colorTextSecondary, marginBottom: 5, backgroundColor: theme["red.3"], borderRadius: 8, paddingLeft: 8, fontSize: 10, fontWeight:600}}>
            Rest Day
          </div>
        )}
        {isHoliday && (
          <div style={{color: theme.colorTextSecondary, backgroundColor: theme["geekblue.3"], marginBottom: 5, borderRadius: 8, paddingLeft: 8, fontSize: 10, fontWeight:600 }}>
            Holiday:{holidayName}
          </div>
        )}
  ...
  const convertToEventData = (
    event: EventData | undefined, // Update the parameter type
  ): EventData => {
    if (!event) {
      return {
        id: '',
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    const newSourceEvents = Array.from(start);
    newSourceEvents.splice(source.index, 1);

    const newFinishEvents = finish ? Array.from(finish) : [];
    newFinishEvents.splice(destination.index, 0, updatedEvent);

    const newScheduledBooks = {
      ...scheduledBooks,
      [source.droppableId]: newSourceEvents,
      [destination.droppableId]: newFinishEvents,
    };
