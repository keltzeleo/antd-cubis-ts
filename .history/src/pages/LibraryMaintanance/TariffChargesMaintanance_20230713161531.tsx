...
const TariffChargesMaintenance: React.FC<TariffChargesMaintenanceProps> = ({
  ...
  const handleSaveAll = () => {
    form.validateFields().then((values) => {
      ...
      message.success("All records updated successfully.");
    });
  };
  ...
  const nestedColumns = [
    ...
    },
    {
      ...
      key: "effectiveDate",
      render: (text, record) => {
        if (record.isEditing || isEditingAll) {
          return (
            <Form.Item
              name={["nestedData", record.key, "effectiveDate"]}
  ...
  const mainTableColumns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        if (record.isEditing || isEditingAll) {
          return (
            <Form.Item
              name={["dataSource", record.key, "effectiveDate"]}
              initialValue={record.effectiveDate}
            >
              <DatePicker />
            </Form.Item>
      ...
      : []),
    {
      ...
      render: () => {
        ...
        return [
          <Button type="primary" onClick={handleEditAll}>
            Edit All
          </Button>,
        ];
      },
  ...
  return (
    <Form form={form} component={false}>
      <EditableProTable<NestedDataType>
        ...
        value={dataSource[0]?.nestedData}
        onChange={(value) =>
          setDataSource((prevDataSource) => [
            { ...prevDataSource[0], nestedData: value },
          ])
        }
        recordCreatorProps={{
        ...
        editable={{
          type: "multiple",
          editableKeys: dataSource[0]?.nestedData?.map((item) => item.key),
          onSave: handleSave,
