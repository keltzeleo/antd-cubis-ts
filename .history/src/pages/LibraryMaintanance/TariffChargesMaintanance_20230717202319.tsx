...
const EditableCell: React.FC<EditableCellProps> = ({
  ...
  const handleNestedDataChange = (
    ...
    updatedRecord[dataIndex] = updatedData;
    setNestedData(updatedData);
    handleSave(updatedRecord); // Add this line to save the updated record
  };
...
const TariffChargesMaintenance: React.FC<TariffChargesMaintenanceProps> = ({
  ...
  const handleAddNestedData = (
    ...
    if (index > -1) {
      const item = newData[index];
      const nestedData = item[dataIndex] as (number | string)[];
      nestedData.push(""); // Push the empty string to the nestedData array
      newData.splice(index, 1, { ...item, [dataIndex]: nestedData }); // Update the item in newData array
      setData(newData);
    }
  };
  ...
  const handleSave = (record: TariffCharge) => {
    const newData = [...data];
    const index = newData.findIndex((item) => record.key === item.key);
