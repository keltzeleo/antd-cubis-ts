...
const EditableTable: React.FC<EditableTableProps> = ({ theme }) => {
  ...
  const convertDataForChart = (data: DataSourceType) => {
    ...
    // Iterate through each block consumption range and rate to create the continuous line
    for (let i = 0; i < blockConsumptions.length; i++) {
      const blockConsumption = blockConsumptions[i];
      const rate = rates[i];
  ...
  const expandableConfig = {
    expandedRowRender: (record: DataSourceType) => {
      ...
      const verticalLine = {
        type: "line",
        data: [
          { x: midPoint, y: 0 }, // Starting point of the line (x, y)
          { x: midPoint, y: 0.3 }, // Ending point of the line (x, y) - Adjust the y-value based on your data range
        ],
        style: {
