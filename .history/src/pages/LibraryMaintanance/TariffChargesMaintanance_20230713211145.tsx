...
const TariffChargesMaintenance: React.FC<TariffChargesMaintenanceProps> = ({
  ...
  const handleEdit = (record: TariffChargesDataType | NestedDataType) => {
            ...
            ? {
                ...
                nestedData: item.nestedData.map((nestedItem) =>
                  nestedItem.key === record.key
                    ? { ...nestedItem, ...values, isEditing: false }
                    : nestedItem
                ),
              }
            : item
      ...
      render: (_, record) => (
        <Space style={{ justifyContent: "space-evenly", width: "100%" }}>
          {record.nestedData?.map((nestedRecord) => (
            <Button
              key={nestedRecord.key}
              type="primary"
              icon={<EditOutlined />}
              onClick={() => handleEdit(nestedRecord, record)}
            >
              Edit
            </Button>
          ))}
        ...
        <Space style={{ justifyContent: "space-evenly", width: "100%" }}>
          <Button
            ...
            onClick={() =>
              handleEdit(
                record,
                dataSource.find((item) => item.nestedData?.includes(record)) ||
                  {}
              )
            }
          />
