...
const TariffChargesMaintenance: React.FC<TariffChargesMaintenanceProps> = ({
  ...
  const handleChangeNewRow = (fieldName: string, value: any) => {
    const [recordKey, fieldKey] = fieldName.split("-");
    setNewRowValues((prevValues) => ({
      ...prevValues,
      [recordKey]: {
        ...(prevValues[recordKey] || {}),
        [fieldKey]: value,
      },
    }));
  };
  ...
  const handleSave = async (key: React.Key) => {
    try {
      ...
      // Update the main record in the dataSource state
      setDataSource((prevDataSource) =>
          ...
          if (record.key === key) {
            ...
            const updatedRecord = {
              ...
              nestedData: record.nestedData?.map((nestedItem) => {
                // Update the specific nested item in the nestedData array
                const nestedFormValues =
                  nestedFormRef.current?.getFieldsValue();
          ...
          return (
            ...
            >
              <ProFormDigit
                ...
                disabled={!record.isEditing}
                onChange={(value) =>
                  handleChangeNewRow(`${record.key}-rate`, value)
                } // Update this line
              />
            </Form.Item>
          );
        }
        // Use the updated value from the form
        const updatedRate = newRowValues[`${record.key}`]?.rate; // Update this line
        // Display the updated rate if available, otherwise use the original value
        const displayedRate =
          updatedRate !== undefined ? updatedRate : record.rate;

        return (
          <span style={{ color: theme.colorText }}>
            RM {record.rate ? renderText(displayedRate.toFixed(2)) : ""} /m³
          </span>
        );
      },
    },
