...
const EditableTable: React.FC<EditableTableProps> = ({ theme }) => {
  ...
  const onSave = async (rowKey: React.Key, record: DataSourceType) => {
    await waitTime(2000);
    console.log("Save:", rowKey, record);
  };

  const onCancel = async (rowKey: React.Key, record: DataSourceType) => {
    console.log("Cancel:", rowKey, record);
  };

  const handleDelete = async (rowKey: React.Key, record: DataSourceType) => {
    await waitTime(2000);
    console.log("Delete:", rowKey, record);
  };
        ...
        return (
          <span
            style={{
              ...
              top: 0,
              left: 0,
              margin: "-10 -6 -10 -8", // Ensure the overlay is behind the content
              backdropFilter: "blur(14px)", // Use backdrop-filter for modern browsers that support it
            }}
          >
            {isEditing ? (
              <>
                <Button
                  type="primary"
                  onClick={() => onSave(record.id, record)}
                >
                  Save
                </Button>
                <Button onClick={() => onCancel(record.id, record)}>
                  Cancel
                </Button>
              </>
            ) : (
