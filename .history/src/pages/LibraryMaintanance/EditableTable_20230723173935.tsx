...
type DataSourceType = {
  ...
  blockConsumption1?: [number, number][] | null; // Update the type to explicitly define it as a tuple array
...
I apologize for the oversight. It seems that the data provided in the defaultData array is incorrect. The blockConsumption1 and blockConsumption2 properties should have values as tuples, not individual numbers.

Here's the corrected data for defaultData:
...
const defaultData: DataSourceType[] = [
  {
    ...
    blockConsumption1: [[0, 19]], // Corrected to use a tuple inside an array
    ratespercubicm1: 0.03,
    blockConsumption2: [[20, 49]], // Corrected to use a tuple inside an array
  ...
  },
  {
    ...
    blockConsumption1: [[0, 15]], // Corrected to use a tuple inside an array
    ratespercubicm1: 0.05,
    blockConsumption2: [[16, 35]], // Corrected to use a tuple inside an array
  ...
  const convertDataForChart = (data: DataSourceType) => {
    ...
    if (data.blocks && data.rates && data.blockConsumption1) {
      data.blocks.forEach((block, index) => {
        if (
          data.rates?.[index] !== undefined &&
          data.blockConsumption1?.[index]
        ) {
          chartData.push({
            block,
            rate: data.rates[index],
            consumption: data.blockConsumption1[index]?.[1] ?? 0,
          });
        }
      });
    }
