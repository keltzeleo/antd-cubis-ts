...
const EditableTable: React.FC = () => {
  ...
  const convertDataForChart = (data: DataSourceType) => {
    const chartData: { blockConsumption: number; rate: number }[] = [];
    ...
    expandedRowRender: (record: DataSourceType) => {
      ...
      const config = {
        ...
        yAxis: {
          ...
          min: 0, // Minimum value for x-axis
          max: 0.2, // Maximum value for x-axis
        },
        color: (value: Datum) => {
          // Modify the parameter type to Datum
          const rate = value.rate as number; // Extract the rate value from Datum
          // Map different colors based on rate values
          if (rate <= 0.03) {
            return "#ff4d4f"; // Red color for rates <= 0.03
          } else if (rate <= 0.06) {
            return "#ffa940"; // Orange color for rates <= 0.06
          } else {
            return "#52c41a"; // Green color for rates > 0.06
          }
        },
      };
