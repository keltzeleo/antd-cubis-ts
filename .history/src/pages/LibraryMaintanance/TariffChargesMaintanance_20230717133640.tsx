  ...
  const handleChangeNewRow = (fieldName: string, value: any) => {
    ...
    setNewRowValues((prevValues) => {
      const updatedNestedData = {
        ...(prevValues[recordKey as string] || {}),
        [fieldKey]: value,
      };
      return {
        ...prevValues,
        [recordKey]: updatedNestedData,
      };
    });
  };
            ...
            {hasNestedRecords && (
              <Button
                ...
                onClick={() =>
                  handleDelete(
                    undefined,
                    record as TariffChargesDataType | undefined
                  )
                }
              >
                Delete
  ...
  const nestedColumns: ProColumns<NestedDataType>[] = [
...

    {
      ...
      render: (_, record) => {
        ...
        const updatedRate =
          newRowValues[record.key as keyof NestedDataType]?.rate;
        const displayedRate =
          updatedRate !== undefined ? updatedRate : record.rate;

        return (
          <span style={{ color: theme.colorText }}>
            RM {displayedRate !== undefined ? displayedRate.toFixed(2) : ""} /m³
          </span>
        );
      },
    },
