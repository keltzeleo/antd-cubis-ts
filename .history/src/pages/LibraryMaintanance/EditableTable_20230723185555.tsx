...
const EditableTable: React.FC = () => {
  ...
  const convertDataForChart = (data: DataSourceType) => {
    ...
    if (
      data.ratespercubicm1 !== undefined &&
      data.ratespercubicm2 !== undefined &&
      data.blockConsumption1 &&
      data.blockConsumption2
    ) {
      const rates1 = data.ratespercubicm1 as number;
      const rates2 = data.ratespercubicm2 as number;
      ...
      if (data.blocks) {
        data.blocks.forEach((block, index) => {
          if (rates1 !== undefined && blockConsumption1?.[1] !== undefined) {
            for (let i = blockConsumption1[0]; i <= blockConsumption1[1]; i++) {
              chartData.push({
                block: `Block Consumption 1 ${block}`,
                rate: rates1.toString(),
                consumption: i,
              });
            }
          }
        ...
        data.blocks.forEach((block, index) => {
          if (rates2 !== undefined && blockConsumption2?.[1] !== undefined) {
            for (let i = blockConsumption2[0]; i <= blockConsumption2[1]; i++) {
              chartData.push({
                block: `Block Consumption 2 ${block}`,
                rate: rates2.toString(),
                consumption: i,
              });
            }
          }
