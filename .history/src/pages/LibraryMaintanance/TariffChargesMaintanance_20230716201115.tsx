  ...
  const handleSave = async (key: React.Key) => {
    try {
      ...
      if (updatedNestedDataSource && updatedNestedData) {
        updatedNestedDataSource.forEach((nestedRecord: NestedDataType) => {
          const updatedNestedRecord = updatedNestedData[nestedRecord.key];
          if (updatedNestedRecord) {
            const { block, rate } = updatedNestedRecord;
            nestedRecord.block = block;
            nestedRecord.rate = rate;
          }
        });
        // Update the nestedData property of the main record with the updated nested data
        const mainRecord = updatedDataSource.find(
          (record) => record.key === key
        );
        if (mainRecord) {
          mainRecord.nestedData = updatedNestedDataSource;
        }
      }
