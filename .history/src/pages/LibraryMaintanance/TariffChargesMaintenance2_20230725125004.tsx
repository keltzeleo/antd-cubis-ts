...
const TariffChargesMaintenance2: React.FC<EditableTableProps> = ({ theme }) => {
  ...
  const [recordsToDelete, setRecordsToDelete] = useState<React.Key[]>([]);
  ...
  const handleDeleteConfirmation = async () => {
    ...
    if (recordsToDelete.length > 0) {
      // Perform the delete action here using the recordsToDelete value
      setDataSource((prevDataSource) =>
        prevDataSource.filter((item) => !recordsToDelete.includes(item.id))
      );
      setRecordsToDelete([]); // Reset the recordsToDelete state after deletion
    }
  };
      ...
      <div style={{ overflowX: "auto", maxWidth: "100%" }}>
        <EditableProTable<DataSourceType>
          ...
          editable={{
            ...
            onDelete: async (rowKey: React.Key, data) => {
              if (rowKey) {
                setRecordsToDelete((prevRecords) => [...prevRecords, rowKey]);
