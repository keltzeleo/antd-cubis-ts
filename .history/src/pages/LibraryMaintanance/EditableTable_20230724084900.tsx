...
const EditableTable: React.FC<EditableTableProps> = ({ theme }) => {
  ...
  const columns: ProColumns<DataSourceType>[] = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <span style={{ color: theme.colorText }}>{record.ratespercubicm3}</span>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (text, record) => (
        <span style={{ color: theme.colorText }}>{record.ratespercubicm4}</span>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (text, record) => (
        <span style={{ color: theme.colorText }}>{record.ratespercubicm5}</span>
      ),
    },
    {
    ...
    },
    {
      ...
      hideInTable: !showBlockConsumptionsRates,
      render: (text, record) => (
        <span style={{ color: theme.colorText }}>{record.ratespercubicm6}</span>
      ),
    },
    ...
    {
      title: "Actions",
      dataIndex: "actions", // Use "actions" key for built-in actions, not "actionButtons"
      valueType: "option",
      fixed: "right",
      width: "138",
      render: (text: any, record: DataSourceType, _, action) => (
        <div className="custom-actions-cell">
          {/* Use "action" from the ProTable actions */}
          {action?.render?.(record, _, {
            editable: action?.startEditable?.(record.id),
          })}
        </div>
      ),
    },
