  ...
  const nestedColumns: ColumnsType<NestedData> = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          <InputNumber
            value={editMode ? record.block[0] : record.block[0]}
            onChange={(value) => {
              const newData = tempData.map((item) => {
                if (item.key === record.key) {
                  return {
          ...
          />
          <InputNumber
            value={editMode ? record.block[1] : record.block[1]}
            onChange={(value) => {
              const newData = tempData.map((item) => {
                if (item.key === record.key) {
                  return {
                    ...
                    nestedData: item.nestedData.map((nestedItem) => {
                      if (nestedItem.key === record.key) {
                        return {
                          ...nestedItem,
                          block: [nestedItem.block[0], value || 0],
                        };
                      }
                      return nestedItem;
                    }),
    ...
    },
    {
      ...
      render: (_, record) => (
        <InputNumber
          value={editMode ? record.rate : record.rate}
          onChange={(value) => {
            const newData = tempData.map((item) => {
              if (item.key === record.key) {
                return {
