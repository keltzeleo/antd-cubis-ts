...
const TariffChargesMaintenance: React.FC<TariffChargesMaintenanceProps> = ({
  ...
  const handleSave = async (key: React.Key) => {
    try {
      ...
      const updatedDataSource = dataSource.map((record) => {
        if (record.key === key) {
          ...
          const updatedRecord = {
            ...
            nestedData: record.nestedData?.map((nestedItem: NestedDataType) => {
              const matchingItem =
      ...
      const savedItems = updatedDataSource.reduce((items: string[], record) => {
        if (
          record.key === key ||
          (record.nestedData &&
            record.nestedData.some(
              (nestedItem: NestedDataType) => nestedItem.key === key
            ))
        ) {
          items.push(`Record ${record.key}`);
          if (record.nestedData) {
            record.nestedData.forEach((nestedItem: NestedDataType) => {
              if (nestedItem.key === key) {
                items.push(
                  `Nested Record ${nestedItem.key} (Record ${record.key})`
                );
              }
            });
          }
      ...
      notification.success({
        message: `Data saved successfully for: ${savedItems.join(", ")}.`,
      });
    } catch (error) {
