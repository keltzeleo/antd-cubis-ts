...
const TariffChargesMaintenance2: React.FC<EditableTableProps> = ({ theme }) => {
  ...
  const handleDeleteConfirmation = async () => {
    ...
    if (recordToDelete) {
      // Perform the delete action here using the recordToDelete value
      setDataSource((prevDataSource) =>
        prevDataSource.filter((item) => item.id !== recordToDelete)
      );
      setRecordToDelete(undefined); // Reset the recordToDelete state after deletion
    }
  };
