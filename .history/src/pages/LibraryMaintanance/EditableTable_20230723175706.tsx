...
type DataSourceType = {
  ...
  blockConsumption2?: [number, number] | null; // Update the type to explicitly define it as a tuple array
...
const defaultData: DataSourceType[] = [
  {
    ...
    blockConsumption1: [0, 20], // Corrected to use a tuple inside an array
    ratespercubicm1: 0.03,
    blockConsumption2: [20, 50], // Corrected to use a tuple inside an array
  ...
  },
  {
    ...
    blockConsumption2: [15, 35], // Corrected to use a tuple inside an array
  ...
  const convertDataForChart = (data: DataSourceType) => {
    ...
    if (data.blocks && data.rates && data.blockConsumption1) {
      data.blocks.forEach((block, index) => {
        const rates = data.rates as string[];
        const blockConsumption1 = data.blockConsumption1 as [number, number][];

        if (
          rates?.[index] !== undefined &&
          blockConsumption1?.[index]?.[1] !== undefined
        ) {
          chartData.push({
            block,
            rate: rates[index],
            consumption: blockConsumption1[index]?.[1] ?? 0,
          });
        }
      });
    }
