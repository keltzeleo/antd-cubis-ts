...
const EditableTable: React.FC = () => {
  ...
  const nestedColumns: ProColumns<BlockRateType>[] = [
    {
      title: "Block",
      dataIndex: "block",
      valueType: "group",
      ...
      <EditableProTable<DataSourceType>
        ...
        expandable={{
          expandedRowRender: (record) => (
            <EditableProTable<BlockRateType>
              rowKey="block"
              headerTitle="Nested Table"
              columns={nestedColumns}
              value={record.blockRate}
              onChange={(newBlockRate) => {
                const newData = [...dataSource];
                const targetIndex = newData.findIndex(
                  (item) => item.id === record.id
                );
                if (targetIndex !== -1) {
                  newData[targetIndex].blockRate = newBlockRate;
                }
              ...
              editable={{
                ...
                editableKeys: editableKeys.filter((key) =>
                  String(key).startsWith(record.id as string)
                ),
