...
interface NestedDataType {
  ...
  modifiedBy: string;
  modifiedDate: string;
  nestedData?: NestedDataType[]; // Add this line
}
    ...
    },
    {
      ...
      render: (_, record) => {
        if (record.isEditing) {
          ...
          const latestNestedData = dataSource.find(
            (item) => item.key === record.key
          )?.nestedData;
          const latestBlock =
            latestNestedData?.[latestNestedData.length - 1]?.block;
          if (latestBlock) {
            return (
              <span style={{ color: theme.colorText }}>
                {`${latestBlock[0]} - ${latestBlock[1]}m³`}
              </span>
            );
          }
          return null; // Handle if no nested data is available
        }
      },
    },
    {
      ...
      render: (_, record) => {
        if (record.isEditing) {
          ...
          const latestRate =
            latestNestedData?.[latestNestedData.length - 1]?.rate;
          if (latestRate) {
            return (
              <span style={{ color: theme.colorText }}>
                RM {renderText(latestRate)} /m³
              </span>
            );
          }
          return null; // Handle if no nested data is available
    ...
    },
    {
      ...
      render: (_, record) => {
        if (record.isEditing) {
          ...
          const latestNestedData =
            record.nestedData?.[record.nestedData.length - 1];
          if (latestNestedData) {
            return (
              <span style={{ color: theme.colorText }}>
                RM{" "}
                {latestNestedData.rate ? renderText(latestNestedData.rate) : ""}{" "}
                /m³
              </span>
            );
          }
