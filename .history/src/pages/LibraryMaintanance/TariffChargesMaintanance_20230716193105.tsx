  ...
  const handleEdit = (
    ...
    setDataSource((prevDataSource) =>
      prevDataSource.map((item) => ({
        ...item,
        isEditing: item.key === (mainRecord?.key || nestedRecord?.key),
        nestedData:
          item.key === mainRecord?.key
            ? item.nestedData?.map((nestedItem) => ({
                ...nestedItem,
                isEditing: nestedItem.key === nestedRecord?.key,
              }))
            : item.nestedData,
      }))
    );
    ...
    setExpandedRowKeys(
      (prevExpandedRowKeys) =>
        new Set<React.Key | undefined>([
          ...Array.from(prevExpandedRowKeys),
          mainRecord?.key || nestedRecord?.key,
        ])
    );
  };
        ...
        expandable={{
          ...
          onExpand: (expanded, record) => {
            if (expanded) {
              ...
              setExpandedRowKeys((prevExpandedRowKeys) => {
                const keys = new Set<React.Key | undefined>(
                  prevExpandedRowKeys
                );
