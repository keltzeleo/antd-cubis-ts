...
const TariffChargesMaintenance: React.FC<TariffChargesMaintenanceProps> = ({
  ...
  const handleSave = async (key: React.Key) => {
    try {
      ...
      // Update values of the new row
      const newRowIndex = updatedDataSource.findIndex(
        (record) => record.key === key
      );
      const newNestedData = updatedDataSource[newRowIndex]?.nestedData;
      if (newNestedData) {
        const newNestedRowIndex = newNestedData.findIndex(
          (nestedItem: NestedItemType) => nestedItem.key === generateUniqueKey()
        );
        if (newNestedRowIndex !== -1) {
          updatedNestedData[`${key}-${newNestedData[newNestedRowIndex].key}`] =
            newNestedData[newNestedRowIndex];
        }
      }
  ...
  const handleAddAdditionalRow = (recordKey: React.Key) => {
    const newData: NestedDataType = {
      key: generateUniqueKey().toString(),
      status: "",
      block: null,
      rate: undefined,
      effectiveDate: "",
