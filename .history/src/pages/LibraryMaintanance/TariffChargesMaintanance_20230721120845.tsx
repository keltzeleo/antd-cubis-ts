...
const TariffChargesMaintenance: React.FC<TariffChargesMaintenanceProps> = ({
  ...
  const columns: ProColumns<TariffChargesDataType>[] = [
    ...
    },
    {
      ...
      key: "rate1",
      render: (_, record) => {
        if (record.isEditing && record.nestedData && record.nestedData[0]) {
          return (
            <Form.Item
              name={[`${record.key}-${record.nestedData[0].key}`, "rate1"]}
          ...
          return (
            <span style={{ color: theme.colorText }}>
              {record.nestedData && record.nestedData[0]
                        ...
                        render: (text, nestedRecord) => {
                          if (nestedRecord.isEditing) {
                            ...
                            return (
                              <span style={{ color: theme.colorText }}>
                                RM{" "}
                                {nestedRecord.rate !== undefined
                                  ? nestedRecord.rate.toFixed(2)
                                  : ""}
                              </span>
                            );
                          }
