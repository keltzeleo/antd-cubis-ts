    ...
    },
    {
      ...
      render: (_, record) => {
        if (editableKeys.includes(record.id)) {
          ...
          return (
            <>
              {record.blockRate?.map((item, index) => (
                <Form.Item
                  key={index}
                  name={[record.id, "blockRate", index, "block"]}
                  initialValue={item.block}
                  rules={[
                    { required: true },
                    () => ({
                      validator(_, value) {
            ...
            <>
              {record.blockRate?.map((item, index) => (
                <span key={index} style={{ marginRight: 8 }}>
                  {item.block ? `${item.block[0]} - ${item.block[1]}m³` : ""}
                </span>
              ))}
            </>
          );
          ...
          return (
            <>
              {record.blockRate?.map((item, index) => (
                <Form.Item
                  key={index}
                  name={[record.id, "blockRate", index, "rate"]}
