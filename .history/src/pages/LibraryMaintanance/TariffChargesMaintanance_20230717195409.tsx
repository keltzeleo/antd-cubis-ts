...
interface EditableCellProps extends React.HTMLAttributes<HTMLElement> {
  ...
  handleAddNestedData: (record: TariffCharge, dataIndex: string) => void;
}
...
const EditableCell: React.FC<EditableCellProps> = ({
  ...
  const [nestedData, setNestedData] = useState<any[]>(record[dataIndex]);

  const handleAddRow = () => {
    const newRow = {};
    setNestedData([...nestedData, newRow]);
  };

  const handleNestedDataChange = (key: string, value: number | string) => {
    const updatedData = nestedData.map((data) => ({
      ...data,
      [key]: data[key] === undefined ? value : data[key],
    }));
    setNestedData(updatedData);
  };

  const handleRemoveRow = (nestedIndex: number) => {
    const updatedData = nestedData.filter((_, i) => i !== nestedIndex);
    setNestedData(updatedData);
  };

  const inputNode = (
    <Input
      value={nestedData[index] ? nestedData[index][dataIndex] : ""}
      onChange={(e) => handleNestedDataChange(dataIndex, e.target.value)}
    />
  );
...
const TariffChargesMaintenance: React.FC = () => {
  ...
  const handleAddNestedData = (record: TariffCharge, dataIndex: string) => {
    const newData = [...data];
