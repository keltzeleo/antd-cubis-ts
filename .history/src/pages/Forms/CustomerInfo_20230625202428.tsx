              ...
              <Form.Item label="Nationality">
                <Select
                  ...
                  filterOption={(input, option) => {
                    const labelMatch = (option?.children as string)
                      ?.toLowerCase()
                      ?.includes(input.toLowerCase());
                    const valueMatch = (option?.value as string)
                      ?.toString()
                      ?.toLowerCase()
                      ?.includes(input.toLowerCase());
                  ...
                  {countries.map((country) => (
                    <Option key={country.value} value={country.value}>
                      <div style={{ display: "flex", alignItems: "center" }}>
                        <WorldFlag
                          code={country.value.toLowerCase()} // Assuming country.value is the country's ISO 3166-1 alpha-2 code
                          height={12}
